...
class ItemListActivity : AppCompatActivity() {
    ...
    override fun onOptionsItemSelected(item: MenuItem?): Boolean {
        when(item?.itemId){
            R.id.menu_sort -> {
                //sort by name or what ever
                swApiObjects?.sortWith(object : Comparator<SwApiObject>{
                    override fun compare(p0: SwApiObject?, p1: SwApiObject?): Int {
                        return if (p0 == null || p1 == null) {
                            0
                        } else {

                            p0.name.compareTo(p1.name, true)
                        }
                    }
                })
                viewAdapter?.notifyDataSetChanged()
            ...
            R.id.show_toast -> {
                Toast.makeText(this, "Congrats Your getting it", Toast.LENGTH_LONG).show()
            }
        }
